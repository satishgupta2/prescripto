        ...
        {appointments.map((item,index)=>(
          <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
            ...
            <div className='flex flex-col gap-2 justify-end'>
             ...
             {item.cancelled && !item.isCompleted &&<button className="sm:min-w-48 py-2 border border-red-500 rounded text-red-500">Appointmnet cancelled</button>}
             {item.isCompleted && <button className='sm:min-w-48 py-2 border text-green-500 border-green-500 rounded'>Completed</button>}
            </div>
          </div>
        )) }
      </div>
